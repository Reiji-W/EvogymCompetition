...
# user@host:path/server/saved_data/ -> <repo>/client/mnt/ を rsync で同期する
...
$RepoRoot   = Split-Path -Parent $ClientRoot                    # .../EvogymCompetition （想定）
...
# Windows 上では /Users/... パスは普通の Test-Path では存在しない扱いになるので、
# 見つかった場合だけ -i オプションを付ける。見つからなければ ssh のデフォルト鍵に任せる。
...
# === rsync のソース/宛先設定 ===
# Bash 版: RSRC="${USER}@${HOST}:${RPATH%/}/server/saved_data/"
...
# Bash 版: rsync -az --delete -e "ssh ${SSH_OPT[*]}" "$RSRC" "$RDST"
& rsync -az --delete `
    -e "ssh $sshOptString" `
    "$RSRC" "$RDST"
